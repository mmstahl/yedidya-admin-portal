...
class PostEventWindow(tk.Toplevel):
    ...
    def _build(self):
        ...
        ttk.Button(tpl_row, text="Show category translations",
                   command=self._on_show_category_translations,
    ...
    # ==================================================================
    # Category translations viewer
    # ==================================================================

    def _on_show_category_translations(self):
        self._status_var.set("Fetching category translations…")
        threading.Thread(
            target=self._run_show_category_translations, daemon=True,
        ).start()

    def _run_show_category_translations(self):
        result = self.action.list_category_translations(env=self.env)
        self.after(0, self._finish_show_category_translations, result)

    def _finish_show_category_translations(self, result):
        self._status_var.set("Ready.")
        if not result.success:
            messagebox.showerror("Lookup failed", result.message, parent=self)
            return
        self._open_category_translations_dialog(result.data or {})

    def _open_category_translations_dialog(self, data):
        """Show a table of Hebrew ↔ English category pairings, with the
        unmatched categories highlighted so the user knows what to fix in
        WPML → Taxonomy Translation → Categories."""
        groups  = data.get('groups',  [])
        orphans = data.get('orphans', [])

        # Build rows: each row is (he_name, en_name, status)
        rows = []
        for g in groups:
            tr = g.get('translations', {}) or {}
            he = tr.get('he')
            en = tr.get('en')
            he_name = he['name'] if he else ''
            en_name = en['name'] if en else ''
            if he and en:
                status = '✓ linked'
            elif he and not en:
                status = '✗ no English translation'
            elif en and not he:
                status = '✗ no Hebrew translation'
            else:
                # Group with neither he nor en (e.g. other language only) — skip.
                continue
            rows.append((he_name, en_name, status))

        # Sort: unpaired first (so they jump out), then pairs alphabetically
        def _key(r):
            return (0 if r[2].startswith('✗') else 1, r[0] or r[1])
        rows.sort(key=_key)

        # ── Dialog ────────────────────────────────────────────────
        dlg = tk.Toplevel(self)
        dlg.title("Category translations")
        dlg.geometry("720x500")
        dlg.transient(self)

        intro = (
            "Each row is one WPML translation group.\n"
            "Categories without a pairing need to be translated in "
            "WPML → Taxonomy Translation → Categories on the site."
        )
        ttk.Label(dlg, text=intro, justify="left",
                  foreground="gray", padding=(12, 10)).pack(anchor="w")
        ...
        tree = ttk.Treeview(
            tree_frame,
            columns=('he', 'en', 'status'),
            show='headings', height=15,
        )
        tree.heading('he',     text='Hebrew')
        tree.heading('en',     text='English')
        tree.heading('status', text='Status')
        tree.column('he',     width=240, anchor='e')   # RTL
        tree.column('en',     width=240, anchor='w')
        tree.column('status', width=200, anchor='w')

        # Red text for unpaired rows
        tree.tag_configure('unpaired', foreground='#b00020')

        for he_name, en_name, status in rows:
            tags = ('unpaired',) if status.startswith('✗') else ()
        ...
        if orphans:
            orphan_frame = ttk.LabelFrame(
                dlg, text="Orphans (not in any WPML translation group)",
                padding=8,
            )
            orphan_frame.pack(fill="x", padx=12, pady=(8, 0))
            for o in orphans:
                lang = o.get('lang') or '?'
                ttk.Label(
                    orphan_frame,
                    text=f"  • {o.get('name', '')}  ({lang}, id={o.get('id')})",
                    foreground='#b00020',
                ).pack(anchor='w')
