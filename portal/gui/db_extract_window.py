...
class DbExtractWindow(tk.Toplevel):
    ...
    def _build(self):
        ...
        # Privacy filter row
        ttk.Label(fields_frame, text="Privacy:").grid(row=2, column=0, sticky="e", **pad)
        self._privacy_var = tk.StringVar(value="—")
        ttk.Combobox(
            fields_frame, textvariable=self._privacy_var,
            values=["—", "Yes", "No"], state="readonly", width=10,
        ).grid(row=2, column=1, sticky="w", **pad)

        # Field list entry
        ttk.Label(fields_frame, text="Fields:").grid(row=3, column=0, sticky="ne", pady=(6, 4), padx=8)
        self._fields_text = tk.Text(fields_frame, height=3, wrap="word",
                                    font=("Consolas", 9))
        self._fields_text.grid(row=3, column=1, columnspan=2, sticky="ew", **pad)
    ...
    def _on_extract(self):
        ...
        privacy_raw = self._privacy_var.get()
        privacy = privacy_raw if privacy_raw != "—" else None
        ...
        self._log_write(f"Gender: {gender_raw}   Privacy: {privacy_raw}\n")
        self._log_write(f"Output: {csv_path}\n\n")

        threading.Thread(
            target=self._run_extract,
            args=(fields, csv_path, gender, privacy),
            daemon=True,
        ).start()

    def _run_extract(self, fields, csv_path, gender, privacy):
        result = self.action.run(fields=fields, csv_path=csv_path, env=self.env,
                                 gender=gender, privacy=privacy)
